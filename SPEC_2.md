...
Переработанная спецификация: сначала данные и админ‑CRUD (источники правды для категорий, промптов и RAG‑документов), затем инструменты и оркестрация агента поверх этой модели. Добавлены жизненный цикл RAG‑файлов и тестовая стратегия (латентность/стоимость/галлюцинации).
...
```json
{ "name": "техподдержка", "synonyms": ["поддержка", "support"], "system_prompt": "Ты специалист техподдержки...", "priority": 10 }
```
...
```json
{ "session_id": "...", "tools_used": ["classify_text","rag_search"], "reply": "...", "sources": [{"id":"...","score":0.81}] }
```
