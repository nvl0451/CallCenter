...
curl -sS --fail "$API_BASE/health" | jq . >/dev/null

if [[ -n "$ADMIN_TOKEN" ]]; then
  echo "[+] Bootstrapping admin defaults (classes, labels, KB)"
  curl -sS --fail -X POST "$API_BASE/admin/bootstrap"     -H "Authorization: Bearer $ADMIN_TOKEN" | jq .
else
  echo "[i] ADMIN_TOKEN not set, skipping /admin/bootstrap"
fi

echo "[+] Features snapshot"
curl -sS --fail "$API_BASE/features" | jq .
...
classify_case() {
  ...
  local sid
  sid=$(curl -sS --fail -X POST "$API_BASE/dialog/start" -H 'Content-Type: application/json' -d '{}' | jq -r .session_id)
  if [[ -z "${sid:-}" || "$sid" == "null" ]]; then
    echo "[!] Failed to start session" >&2; exit 1
  fi
  echo "[+] Send message: $message"
  local resp
  resp=$(curl -sS --fail -X POST "$API_BASE/dialog/message" -H 'Content-Type: application/json'     -d "$(jq -n --arg s "$sid" --arg m "$message" '{session_id:$s, message:$m}')")
  echo "$resp" | jq .
  local typ
...
curl -sS --fail -X POST "$API_BASE/rag/ingest" -H 'Content-Type: application/json' -d '{}' | jq .

echo "[+] RAG ask"
RAG_RESP=$(curl -sS --fail -X POST "$API_BASE/rag/ask" -H 'Content-Type: application/json'   -d '{"question":"какая политика возврата?","top_k":4}')
echo "$RAG_RESP" | jq .
